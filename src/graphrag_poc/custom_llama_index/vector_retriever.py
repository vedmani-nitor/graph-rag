    ...
    def _get_nodes_with_score(
        ...
        for i, triplet in enumerate(triplets):
            ...
            if self.include_properties:
                ...
                text = f"{triplet[0].id} (Description: {triplet[0].properties.get('description', '')}) -> {triplet[1].id} (Description: {triplet[1].properties.get('description', '')}) -> {triplet[2].id} (Description: {triplet[2].properties.get('description', '')})"
            results.append(
                NodeWithScore(
                    node=TextNode(
                        text=text,
